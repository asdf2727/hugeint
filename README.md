...
A C++ class that adds arbitrary precision integers aimed at high performance and ease of use.
All you need to do is replace int with hugeint and the rest is done for you!
...
- ### Constructors and Assignments
  ...
  hugeint num5(4.2e10);     // floating point constructor/assignment. Works with floats and doubles.
...
- ### String Assignment
  ...
  The conversion between string and hugeint can be done in hexadecimal, decimal, octal or binary. The base is determined
  automaticly when converting.
  ...
  >- binary notation: ```0b1337```, ```b1337```, ```1337b```.
    ...
    - Spaces (``` ```) or apostrophes (```'```) are allowed at any point inside the number, exluding prefixes or
      suffixes.

    - Any number of `+` and `-` symbols can be present, as long as they are at the beginning of the string. For
      example, ```-x2d9``` is allowed, but ```x-2d9``` is not.

    - **Warning:** leading zeros are allowed, but if the first character after the sign is a zero and the next is a
      decimal
      digit, octal base is assumed.
...
- ### Boolean Logic
  ...
  I also added ```&&``` and ```||```,
  but they work by first casting both numbers to bool and then doing the operation.
...
- ### Incrementing and Decrementing
  ...
  It includes ```++``` and ```--```, so both pre- and post- increment or decrement.
  All 4 work as expected.
...
- ### Adition and Subtraction
  ...
  It includes ```+```, ```+=```, ```-```, ```-=``` and the unary versions of ```+``` and ```-```.
  All of them work as expected.
  Unary ```+``` does nothing and unary ```-``` negates the number.
...
The library also includes some extra functions for ease of use, with more coming in the future.
If you have any requests, you can find my mail in the bio of my profile.

- ### String functions

    - ```size_t fromString (std::string::const_iterator begin, std::string::const_iterator end)``` - the same as an
      assignment of std::string
...
- ### Bit Functions
  ...
  A more direct way of modifying and reading bits of the number.
  All functions allow positions outside written values (i.e. leading zeros), but due to two's complement negative
  numbers have leading ones.
...
- ### Extra Functions
  ...
  These functions add extra functionality to the class.
  They can be called in two ways, either as member functions or as extrnal functions.
  ...
  **note:** All external functions are declared in the 'huge' namespace to avoid ambiguity bewteen std functions and
  hugeint custom ones.

    - #### Power

      It allows exponentiation, with or without modulus. Functions allow any T type castable to uint64_t.

        - ```template <typename T> hugeint pow (T exponent)``` - member function. raises 'this' to the exponent.
        - ```template <typename T> hugeint huge::pow (hugeint base, T exponent)``` - external function. returns base ^
          exponent.
        - ```template <typename T> hugeint pow (T exponent, const hugeint &modulus)``` - member function. raises 'this'
          to the exponent, then aplies the modulo.
        - ```template <typename T> hugeint huge::pow (hugeint base, T exponent, const hugeint &modulus)``` - external
    ...
    - #### Mathematical Functions
        ...
        - ```hugeint rand (size_t size, bool rand_sign)``` - member function. Turns 'this' into a random value
          with size bits. The sign is positive if rand_sign is false, otherwise it is also randomised.
        - ```hugeint huge::rand (size_t size, bool rand_sign)``` - external function. Returns a random value with size
          bits. The sign is positive if rand_sign is false, otherwise it is also randomised.
        - ```hugeint gcd (const hugeint &other)``` - member function. Turns 'this' into the greatest common divisor
          between itself and 'other'.
        - ```hugeint huge::gcd (const hugeint &num1, const hugeint &num2)``` - external function. returns the greatest
          common divisor between num1 and num2.
        - ```hugeint lcm (const hugeint &other)``` - member function. Turns 'this' into the least common multiple
          between itself and 'other'.
        - ```hugeint huge::lcm (const hugeint &num1, const hugeint &num2)``` - external function. returns the least
          common multiple between num1 and num2.

    - #### Roots

        - ```hugeint sqrt ()``` - member function. Turns 'this' into the floor of the square root of num.
        - ```hugeint huge::sqrt (const hugeint &num)``` - external function. Returns the floor of the square root of
          num.
        - ```hugeint cbrt ()``` - member function. Turns 'this' into the floor of the cube root of num.
        - ```hugeint huge::cbrt (const hugeint &num)``` - external function. Returns the floor of the cube root of num.
        - ```hugeint nthroot (T degree)``` - member function. Turns 'this' into the floor of the n-th degree root of
          num. T can be any type that casts to uint64_t.
        - ```hugeint huge::nthroot (const hugeint &num, T degree)``` - external function. Returns the floor of the n-th
          degree root of num. T can be any type that casts to uint64_t.
...
If you otherwise wish to contact me, you can find my mail in the bio of my profile.

I hope you'll enjoy working with it as much as I did making it.
Thank you for using this library and good luck on your project!